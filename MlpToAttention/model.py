...
# --------------------------------------------------------------------------
# 2. Spatial Stream: Transformer Block (Attention-based)
#    [Modification] Replaced MLP Mixer with Self-Attention
# --------------------------------------------------------------------------
class SpatialTransformerBlock(nn.Module):
    def __init__(self, dim, num_joints, num_heads=4, dropout=0.1):
        super().__init__()
        self.dim = dim
        self.num_joints = num_joints
        self.num_heads = num_heads
        
        # 1. Spatial Attention (Replaces Token Mixing MLP)
        self.norm1 = RMSNorm(dim)
        self.attn = nn.MultiheadAttention(embed_dim=dim, num_heads=num_heads, dropout=dropout, batch_first=True)
        
        # 2. Feed Forward Network (Standard Transformer FFN)
    ...
    def forward(self, x):
        ...
        # Reshape for Spatial Attention: Treat (N*T) as batch, V as sequence length
        # (N, T, V, D) -> (N*T, V, D)
        x_reshaped = x.reshape(N * T, V, D)
        
        # 1. Self-Attention (Spatial Interaction)
        residual = x_reshaped
        ...
        # 2. FFN (Channel Mixing)
        residual = x_reshaped
        x_norm = self.norm2(x_reshaped)
        x_ffn = self.ffn(x_norm)
        x_reshaped = residual + x_ffn
        
        # Restore shape: (N*T, V, D) -> (N, T, V, D)
        x = x_reshaped.reshape(N, T, V, D)
...
# --------------------------------------------------------------------------
# Main Model: ST_Model (Using Spatial Transformer Blocks)
# --------------------------------------------------------------------------
class ST_Model(nn.Module):
    def __init__(self, 
        ...
        # Attention Heads 설정 (기본값 4 사용)
        num_heads = 4

        # --- Stage 1 ---
        # [Mod] SpatialMixerBlock -> SpatialTransformerBlock
        self.spatial_1 = SpatialTransformerBlock(hidden_dim, num_joints, num_heads=num_heads, dropout=dropout)
        # Shift 없음: [0, 0]
        self.temporal_1 = SwinTemporalBlock(hidden_dim, num_heads=num_heads, window_sizes=window_sizes, shift_sizes=[0, 0], dropout=dropout, bottleneck_ratio=0.5)
        ...
        # --- Stage 2 ---
        # [Mod] SpatialMixerBlock -> SpatialTransformerBlock
        self.spatial_2 = SpatialTransformerBlock(hidden_dim, num_joints, num_heads=num_heads, dropout=dropout)
        # Shift 적용: [5, 10]
        self.temporal_2 = SwinTemporalBlock(hidden_dim, num_heads=num_heads, window_sizes=window_sizes, shift_sizes=[ss_small, ss_large], dropout=dropout, bottleneck_ratio=0.5)
        
        # --- Stage 3 ---
        # [Mod] SpatialMixerBlock -> SpatialTransformerBlock
        self.spatial_3 = SpatialTransformerBlock(hidden_dim, num_joints, num_heads=num_heads, dropout=dropout)
        # Shift 없음: [0, 0]
        self.temporal_3 = SwinTemporalBlock(hidden_dim, num_heads=num_heads, window_sizes=window_sizes, shift_sizes=[0, 0], dropout=dropout, bottleneck_ratio=0.5)
