...
class NTURGBDDataset(Dataset):
    ...
    def _get_augmented_view(self, features):
        ...
        if np.random.rand() < config.PROB:
            ...
            # [수정] 하드코딩된 숫자 대신 config 값 사용 (해결책 2)
            # config.NUM_COORDS는 21이고, 이를 3으로 나누면 벡터의 개수(7)가 됩니다.
            reshaped = feat.view(T, config.NUM_JOINTS, config.NUM_COORDS // 3, 3) 
            rotated = torch.matmul(reshaped, rot_mat.T)
            
            # [수정] 회전 후 다시 원래 차원으로 복구할 때도 config 값 사용
            feat = rotated.view(T, config.NUM_JOINTS, config.NUM_COORDS)
