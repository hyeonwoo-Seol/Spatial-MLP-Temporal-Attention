...
# ##--------------------------------------------------------------------------
# 1. 21차원 입력 (All Vectors) -> [MODIFIED] 15차원 입력
...
#    - [MODIFIED] Group E: Spine Base to Extremities (3) - 손/발의 척추 중심 기준 벡터
...
def _calculate_features(coords):
    """
    특징 계산 함수 
    전체 채널 수: Group A(3) + Group B(3) + Group C(3) + Group D(3) + Group E(3) = 15 Channels
    """
    T = coords.shape[0]
    if T == 0:
    ...
    # [MODIFIED] Group E: Spine Base to Hands/Feet Vectors
    # 기존 Hand-to-Hand/Foot (9채널) 삭제 후 대체
    # 인덱스: 0(SpineBase), 7(L.Hand), 11(R.Hand), 15(L.Foot), 19(R.Foot)
    
    # 1. Vectors relative to SpineBase (0)
    vec_sb_lh = coords[:, :, 7, :] - coords[:, :, 0, :]   # L.Hand
    vec_sb_rh = coords[:, :, 11, :] - coords[:, :, 0, :]  # R.Hand
    vec_sb_lf = coords[:, :, 15, :] - coords[:, :, 0, :]  # L.Foot
    vec_sb_rf = coords[:, :, 19, :] - coords[:, :, 0, :]  # R.Foot
    
    # 정규화
    vec_sb_lh = vec_sb_lh / safe_spine_len[:, :, 0, :]
    vec_sb_rh = vec_sb_rh / safe_spine_len[:, :, 0, :]
    vec_sb_lf = vec_sb_lf / safe_spine_len[:, :, 0, :]
    vec_sb_rf = vec_sb_rf / safe_spine_len[:, :, 0, :]
    
    # 담당 관절에 할당 (나머지 0)
    # 4개의 벡터를 각각의 관절 노드에 배치하므로 3채널로 충분함 (서로 인덱스가 겹치지 않음)
    group_e_features = np.zeros((T, coords.shape[1], BASE_NUM_JOINTS, 3))

    group_e_features[:, :, 7, :] = vec_sb_lh
    group_e_features[:, :, 11, :] = vec_sb_rh
    group_e_features[:, :, 15, :] = vec_sb_lf
    group_e_features[:, :, 19, :] = vec_sb_rf
    
    # Concatenation (Total 15 Channels)
    # 주의: config.NUM_COORDS 값이 21에서 15로 변경되어야 합니다.
    final_features_per_person = np.concatenate([
        bone_features,          # Group A (3)
        velocity_features,      # Group B (3)
        rel_center_features,    # Group C (3)
        rel_other_features,     # Group D (3)
        group_e_features        # Group E (3)
    ], axis=-1)
...
def calculate_and_save_stats():
    print("--- Calculating Stats for Features (Separate for X-Sub / X-View) ---")
