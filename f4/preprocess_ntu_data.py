...
# - SkateFormer 방식 적용: 유효 프레임 추출 후 전체를 64프레임(MAX_FRAMES)으로 Resize.
# - 좌표(Coordinate) 단계에서 Resize 수행 후 Feature 계산 (속도 정보 정합성 유지).
# ##----------------------------------------------------------------------------
...
def resize_data_skateformer_style(data_numpy, target_frames=MAX_FRAMES):
    ...
    Valid Frames(움직임이 있는 구간)만 추출하여 Target Frames로 Resize(Linear Interpolation)합니다.
    Input: (T, M, V, C) numpy array
    Output: (target_frames, M, V, C) numpy array
    """
    T, M, V, C = data_numpy.shape
    
    # 1. Valid Frame 판별 (SkateFormer: sum of all joints != 0)
    # (T, M, V, C) -> (T,)
    valid_mask = np.sum(np.abs(data_numpy).sum(axis=1).sum(axis=1).sum(axis=1), axis=0) != 0
    valid_indices = np.where(valid_mask)[0]
    
    if len(valid_indices) == 0:
        # 데이터가 아예 없으면 0으로 채워서 반환
    ...
    # torch.nn.functional.interpolate는 (Batch, Channel, Length) 형태를 원함
    # 따라서 (Real_T, M, V, C) -> (1, M*V*C, Real_T) 로 변환
    ...
    # 다시 원래 형태로 복구: (1, M*V*C, Target_T) -> (Target_T, M, V, C)
...
def _calculate_features(coords):
    ...
    # 이미 Resize된 좌표에서 속도를 구하므로 물리적으로 왜곡될 수 있으나,
    # SkateFormer 등의 논문에서는 이 방식을 사용하여 동작의 동적 특성을 학습합니다.
    velocity = np.zeros_like(coords)
    velocity[1:] = coords[1:] - coords[:-1]
    # Resize로 인해 시간 간격(dt)이 달라졌지만, 여기서는 상대적인 변화량 패턴이 중요하므로 그대로 사용
...
def process_file_for_stats(filename):
    """통계 계산용 워커 함수: X-Sub, X-View 여부를 함께 반환"""
    ...
    # [SkateFormer Strategy Apply]
    # 통계 계산 전, 좌표 자체를 Resize하여 학습 데이터 분포와 일치시킴
...
def process_and_save_file(filename):
    ...
    if coords.shape[0] == 0:
        ...
        # [SkateFormer Strategy Apply]
        # 원본 좌표를 읽어 Valid Crop -> Resize (64 Frames) 수행
        resized_coords = resize_data_skateformer_style(coords, target_frames=MAX_FRAMES)
        
        # Resize된 좌표를 기반으로 특징(Bone, Velocity 등) 계산
        feat = _calculate_features(resized_coords) 
        
        # 만약 어떤 이유로 MAX_FRAMES보다 짧거나 길다면 (resize 함수에서 보장하지만 안전장치)
        if feat.shape[0] != MAX_FRAMES:
             # 재귀적으로 다시 맞추거나 Padding/Cutting (여기서는 단순 Cut/Pad)
             # resize_data_skateformer_style이 정확하다면 이 분기는 거의 타지 않음
             pass
...
def main():
    ...
    print(f"--- Processing All Files (SkateFormer Strategy: Resize to {MAX_FRAMES} frames) ---")
    filenames = os.listdir(SOURCE_DATA_PATH)
    num_cores = cpu_count() - 1 if cpu_count() > 1 else 1
